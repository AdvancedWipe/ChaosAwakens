...
public class FoodComponents {
  ...
  static {
    ...
    COOKED_BACON = build(14, 1.0f, true, false, true).statusEffect(
            new StatusEffectInstance(StatusEffects.STRENGTH, 2000, 0), 1.0f)
    ...
    SPARK_FISH = build(3, 0.2f, false, false, true).statusEffect(
        new StatusEffectInstance(StatusEffects.FIRE_RESISTANCE, 100, 0), 1.0f).build();
    LAVA_EEL = build(2, 0.6f, false, false, true).statusEffect(
        new StatusEffectInstance(StatusEffects.FIRE_RESISTANCE, 600, 0), 1.0f).build();
  ...
  private static FoodComponent.Builder build(int hunger, float saturation) {
    return build(hunger, saturation, false);
  }

  private static FoodComponent.Builder build(int hunger, float saturation, boolean meat) {
    return build(hunger, saturation, meat, false);
  }

  private static FoodComponent.Builder build(int hunger, float saturation, boolean meat,
      boolean snack) {
    return build(hunger, saturation, meat, snack, false);
  }

  private static FoodComponent.Builder build(int hunger, float saturation, boolean meat,
      boolean snack, boolean alwaysEdible) {
